...
try:
    ...
    with open(data_path + "/mhc_questions.json", "r", encoding="utf-8") as f:
        for l in f:
            mhc_question = Question.parse_raw(l)
            mhc_questions.append(mhc_question)
    with open(data_path + "/mhc_all_metadatas.json", "r", encoding="utf-8") as f:
        for l in f:
            mhc_meta = json.loads(l)
            mhc_all_metadatas.append(mhc_meta)
    with open(data_path + "/mhc_embeddings.npy", "rb") as f:
...
def match(match_body: MatchBody) -> MatchResponse:
    ...
    questions, matches, query_similarity = match_instruments(
        match_body.instruments, query, mhc_questions, mhc_all_metadatas, mhc_embeddings
    )
